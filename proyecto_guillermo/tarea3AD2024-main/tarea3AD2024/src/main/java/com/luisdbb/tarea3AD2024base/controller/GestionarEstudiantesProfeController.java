/**
 *Clase GestionarEstudiantesProfeController.java
 * 
 ...
 *@see SesionService
 *
 *Controller de la pantalla de gestión de estudiantes para el perfil Profesorado.
 */
...
@Controller
public class GestionarEstudiantesProfeController implements Initializable {
	@FXML
	...
	/**
	 * Tutor de centro actualmente logueado. Se obtiene de
	 * {@link SesionService}.
	 */
	...
	/** Códigos de curso del departamento de Informática y Comunicaciones. */
	...
	/**
	 * Configura las columnas de la tabla con {@code PropertyValueFactory} y
	 * añade el listener de selección para rellenar el formulario.
	 */
	...
	private void cargarDatos() {
	    if (tutorLogueado != null) {
	        datos.setAll(estudianteService
	                .findByProfesoradoId(tutorLogueado.getIdUsuario()));
	    }
	}

	/**
	 * Rellena el formulario con los datos del estudiante seleccionado.
	 *
	 * @param e estudiante seleccionado en la tabla
	 */
	...
	/**
	 * Crea un nuevo estudiante asignándolo automáticamente al tutor logueado.
	 *
		   ...
		   if (!validar(true)) return;
		    try {
		        Estudiante nuevo = new Estudiante(
		                txtUsuario.getText().trim(), passContrasena.getText(),
		                txtNombre.getText().trim(), txtApellidos.getText().trim(),
		                cboCurso.getValue() != null ? cboCurso.getValue() : "");
		        nuevo.setProfesorado(tutorLogueado);
		        estudianteService.save(nuevo);
		        cargarDatos();
		        limpiar(null);
		        ok("Estudiante creado y asignado a tu grupo correctamente.");
		    } catch (Exception ex) {
	...
	/**
	 * Elimina el estudiante seleccionado tras confirmación. Solo puede eliminar
	 * estudiantes de su propio grupo.
	 *
	 ...
	 *
	 * @param esNuevo {@code true} si es una creación (exige contraseña)
	...
	private boolean validar(boolean esNuevo) {
		...
		if (esNuevo && passContrasena.getText().isBlank()) {
